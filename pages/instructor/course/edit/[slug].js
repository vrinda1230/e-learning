...
const CourseEdit = () => {
  ...
  const handleDrop= async (e, index)=>{
    ...
    //save the new order of lessons in DB 
    const {data} = await axios.put(`/api/course/${slug}`,{
      ...values,
      image,
    });
  ...
  const handleVideo = async (e)=>{
    ...
    //save progress bar and send data to backend
  ...
  const handleUpdateLesson = async (e)=>{
    //console.log("handle lesson update");
    e.preventDefault();
  ...
  return (
    ...
    <InstructorRoute>
      ...
      <h1 className="jumbotron text-center square pb-5 pt-5">Update Course</h1>
